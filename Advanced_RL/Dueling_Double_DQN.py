...
class DQN:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, learning_rate, gamma, epsilon, target_update, device, dqn_type='VanillaDQN'):
        ...
        if dqn_type in ('DuelingDQN', 'DuelingDoubleDQN'):
    ...
    def update(self, transition_dict):
        ...
        # 下个状态的最大Q值
        with torch.no_grad():
            if self.dqn_type in ('DoubleDQN', 'DuelingDoubleDQN'):
        ...
        dqn_loss.backward()  # 反向传播更新参数,计算梯度
        self.optimizer.step() #Adam按照梯度修改参数
...
agent = DQN(state_dim, hidden_dim, action_dim, lr, gamma, epsilon,
            target_update, device, "DuelingDoubleDQN")
...
plt.xlabel('Episodes')
plt.ylabel('Returns')
plt.title('Dueling Double DQN on {}'.format(env_name))
plt.show()
